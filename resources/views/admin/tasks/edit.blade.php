                    ...
                    <form action="{{ route('admin.tasks.update', $task) }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                            ...
                            <div>
                                ...
                                <x-text-input id="platform" class="block mt-1 w-full" type="text" name="platform" :value="old('platform', $task->platform)" required />
                            </div>

                            <div>
                                <x-input-label for="reward" :value="__('Reward (USDT)')" />
                                <x-text-input id="reward" class="block mt-1 w-full" type="number" name="reward" :value="old('reward', $task->reward)" step="0.01" min="0" required />
                            </div>

                            <div>
                                <x-input-label for="time_required" :value="__('Time Required')" />
                                <x-text-input id="time_required" class="block mt-1 w-full" type="text" name="time_required" :value="old('time_required', $task->time_required)" placeholder="e.g. 5-10 minutes" required />
                            </div>
                            ...
                            <div>
                                ...
                                <select id="vip_level_required" name="vip_level_required" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full">
                                    @foreach($vipLevels as $vipLevel)
                                        <option value="{{ $vipLevel->id }}" {{ old('vip_level_required', $task->vip_level_required) == $vipLevel->id ? 'selected' : '' }}>
                            ...
                            <div class="col-span-2">
                                ...
                                <textarea id="description" name="description" rows="5" class="border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm block mt-1 w-full" required>{{ old('description', $task->description) }}</textarea>
                                <p class="text-sm text-gray-500 mt-1">Provide detailed instructions for completing this task.</p>
                            </div>

                            <div class="col-span-2">
                                <label class="inline-flex items-center">
                                    <input type="checkbox" name="is_active" value="1" class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500" {{ old('is_active', $task->is_active) ? 'checked' : '' }}>
